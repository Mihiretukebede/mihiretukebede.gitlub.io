---
title: "Extreme Gradient Boosting and Naive Bayes Machine learning algorithms for for facilitating title/abstract screening in systematic reviews: predicting inclusion/exclusion of abstracts"
...
description: |
  In this short blog post, I would like to try machine learning methods for automated title/abstract screening in systematic reviews. 
...
library(dplyr)
library(bib2df)
library(xgboost) #for converting bib file to data frame
library(lime) #for plotting most frequent words
library(bib2df) # for reading and converting bibliographic data to R data frame
...
covid <- bib2df("covid19.bib") %>% # improt
  filter(!is.na(ABSTRACT)) %>% # select only papers with no missing abstract
  filter(CATEGORY=="ARTICLE") %>% # Only journal articles
...
![Train and Test Set in Python Machine Learning - How to Split - DataFlair](https://data-flair.training/blogs/wp-content/uploads/sites/2/2018/08/1-16.png)
...
![Top Machine Learning Framework: 5 Machine Learning Frameworks of 2019](https://d3l69s690g8302.cloudfront.net/wp-content/uploads/2019/10/01100418/Machine-learning-frameworks.jpeg)
...
It resulted 93.8% accuracy. In real world setting this may not happen because we will work on highly related titles and abstracts. My experiment brings two somehow related documents (SARS and covid19). ALthough we are explcitly looking for covid papers, I assigned some of the covid papers to be excluded just to add some confusion.

Let's pick two abstracts(Abstract #89 and #273) and see what are the most important terms of the abstract that xgboost used for its predictions. We will use *lime* package to explain the predictions
...
ab_to_explain <- head(test[273,]$ABSTRACT, 6)

explainer <- lime::lime(ab_to_explain, model = xgb_model, 
                  preprocess = get_matrix, tokenization = default_tokenize)

explanation <- lime::explain(ab_to_explain, explainer, n_labels=1,
                       n_features = 7) # Set number most important features to 7
```
...
test_data <- test[, c(5:6)] 

test_corpus <- corpus(test$ABSTRACT, 
                     docvars = data.frame(abstract_BIBTEX = names(test_data)))
...
```{r, cache=T, include=T, echo=T, message=FALSE, warning=FALSE}
docidtest <- paste(test$BIBTEXKEY)
...
Our model predicted the test data set with 87% accuracy. This is woderful! The other model performance parameters are also very promising.

One limitation of implementing this model using *quanteda.textmodels* is we cannot benefit the great advantages of *lime* package, we cannot use *lime* to explain our naive\_bayes model locally (within abstracts) as we did it our prediction using *XGB* because *lime* currently doesn't support *quanteda.textmodels* package. Currently *lime* package only explains models developed using `mlr`, `xgboost`, `h2o`, `keras`, or `MASS`packages.

# Final remarks

I see lots of promises in NLP for systematic reviews. However, the process is quite complex and we don't know which model works best unless we experiment several models with lots of different model parameter tuning. I realized its tremendous potential even after such a tiny scratch on its surface.

# Next stop

![Next Stop: Growing Successful -- Brian Leichnitz](https://brianleichnitz.files.wordpress.com/2017/06/maxresdefault.jpg)
